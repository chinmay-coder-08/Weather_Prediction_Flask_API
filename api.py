...
load_dotenv()
app = Flask(__name__)
CORS(app, origins=["https://coolweatherinfo.netlify.app"])
...
def get_current_weather(city):
    if city.lower() == "nanded":
        url = "https://api.openweathermap.org/data/2.5/weather?id=1260022&units=metric&appid=bb49fd938c370f56c61c5e7b541a82e8"
    else:
...
def predict_future(model, current_value, feature_name):
    ...
    df = pd.DataFrame({feature_name: [current_value]})
    predictions = [current_value]
    for _ in range(5):
        next_value = model.predict(df)[0]  # Predict with correct format
        predictions.append(next_value)
        # Update input format for next step
        df = pd.DataFrame({feature_name: [next_value]})
...
def predict():
    ...
    compass_points = ["N", "NNE", "NE", "ENE", "E", "ESE", "SE",
                      "SSE", "S", "SSW", "SW", "WSW", "W", "WNW", "NW", "NNW"]
    compass_direction = compass_points[int(wind_deg / 22.5) % 16]
    compass_direction_encoded = le.transform([compass_direction])[
        0] if compass_direction in le.classes_ else -1
    ...
    current_df = pd.DataFrame([current_data])[
        ["MinTemp", "MaxTemp", "WindGustDir", "Humidity", "Pressure", "Temp", "WindGustSpeed"]]

    # Make predictions
    rain_prediction = rain_model.predict(current_df)[0]
    future_temp = predict_future(temp_model, current_weather['Temp'], "Temp")
    future_hum = predict_future(
        hum_model, current_weather['Humidity'], "Humidity")
